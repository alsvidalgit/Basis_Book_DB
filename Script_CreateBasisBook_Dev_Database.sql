...
-- Servidor: SQL
-- Database: BasisBookDb_Dev
-- =====================================================

-- IMPORTANTE: Execute este script no SQL Server Management Studio ou sqlcmd

-- 1. Criar a database se não existir
IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = N'BasisBookDb_Dev')
BEGIN
    CREATE DATABASE [BasisBookDb_Dev];
END
GO
...
WHERE TABLE_CATALOG = 'BasisBookDb_Dev' AND TABLE_NAME NOT LIKE 'VwRel%' AND TABLE_TYPE = 'BASE TABLE';
GO

PRINT 'Database BasisBookDb_Dev criada com sucesso no SQL Server Express!';
GO
